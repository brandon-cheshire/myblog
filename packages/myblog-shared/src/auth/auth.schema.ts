...
export const twoFactorAuthenticationCodeSchema = z.object({
  twoFactorAuthenticationCode: z
    .string()
    .min(1, 'Two-factor authentication code is required'),
});

export type TwoFactorAuthenticationCode = z.infer<
  typeof twoFactorAuthenticationCodeSchema
>;
...
export const changePasswordSchema = z.object({
  ...
  newPassword: z
    ...
    .refine(isValidPassword, {
      message:
        'New password must be at least 10 characters long and contain at least one uppercase letter, one lowercase letter, one digit, and one special character',
    }),
});
