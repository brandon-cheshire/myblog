...
if (!process.env.DATABASE_URL) {
  console.error('DATABASE_URL is not set. Set it in packages/myblog-backend/.env');
  process.exit(1);
}

const { initLogging } = await import('../../common/utils/app-logger/init-logging.js');
...
async function seed() {
  ...
  const { UserWithThatEmailAlreadyExistsException } = await import(
    '../../exceptions/UserWithThatEmailAlreadyExistsException.js'
  );
  ...
  try {
    ...
    if (err instanceof UserWithThatEmailAlreadyExistsException) {
      console.log(`User already exists: ${email}. Set SEED_FORCE=true to replace.`);
      return;
    }
    throw err;
  }
